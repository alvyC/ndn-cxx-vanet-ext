...
PLocation::PLocation() = default;

PLocation::PLocation(double longitude, double latitude)
...
size_t
PLocation::wireEncode(EncodingImpl<TAG>& encoder) const
{
  std::cout << "PLocation wireEncode()" << std::endl;
  ...
  totalLength += encoder.prependVarNumber(ndn::lp::tlv::PLocation);

  return totalLength;
}

NDN_CXX_DEFINE_WIRE_ENCODE_INSTANTIATIONS(PLocation);
...
void
PLocation::wireDecode(const Block& wire)
{
  std::cout << "PLocation wireDecode()" << std::endl;

  if (wire.type() != ndn::lp::tlv::PLocation) {
